      ...
      ffmpeg.ffprobe(animatedMediaPath, (err, metadata) => {
        ...
        const command = ffmpeg()
          ...
          .outputOptions([
            ...
            // Telegram特定的VP9编码参数
            '-profile:v', '0',           // VP9 Profile 0 (8-bit)
            '-level', '3.1',             // VP9 Level 3.1
            '-tile-columns', '1',        // 适合小视频的tile设置
            '-tile-rows', '0',
            '-frame-parallel', '0',      // 禁用帧并行以提高兼容性
            '-auto-alt-ref', '1',        // 启用alt-ref帧
            '-lag-in-frames', '16',      // 适中的lag设置
            ...
            '-f', 'webm',
            '-dash', '1',                // 启用DASH兼容性
            '-cluster_time_limit', '5000', // 集群时间限制（毫秒）
            '-cluster_size_limit', '2M',   // 集群大小限制
            
            // 元数据设置（确保Telegram正确识别）
            '-metadata', 'title=Animated Quote',
            '-metadata', 'comment=Generated by QuoteAPI',
            '-metadata', 'encoder=FFmpeg libvpx-vp9',
            ...
            '-row-mt', '1',
            '-threads', '4',
            '-static-thresh', '0',       // 禁用静态阈值
            '-max-intra-rate', '300',    // 限制关键帧比特率
            '-quality', 'good',          // 编码质量模式
            '-speed', '1',               // 编码速度（1=慢但高质量）
